...
def test_upload_pdf():
    ...
    with open("test.pdf", "rb") as f:
        ...
        resp = requests.post(f"{BASE_URL}/rag/upload", files=files)
...
def test_search():
    ...
    for q in queries:
        resp = requests.get(f"{BASE_URL}/rag/search", params={"q": q, "top_k": 2})
        if resp.status_code == 200:
            data = resp.json()
            print(f"\n搜索: {q}")
...
def test_rag_ask():
    ...
    for q in questions:
        print(f"\n问题: {q}")
        resp = requests.get(f"{BASE_URL}/rag/ask", params={"question": q})
        if resp.status_code == 200:
            data = resp.json()
