...
| 计划项 | 现状 | 结论 |
|--------|------|------|
| Three.js 渲染 | `web/sim.html` 已有 Canvas 2D 鸟瞰渲染（仓库+小车+yaw 旋转） | 2D → 3D 升级 |
| SSE 位置推送 | `app/main.py` 已有 SSE（agent 流式输出）；sim 位置目前靠轮询 `/api/sim/status` | 新增 sim 位置流 |
| 点击地图交互 | `web/sim.html` 已有 canvas click → 坐标 | 迁移到 3D raycast |
| 货架/装卸区模型 | `app/sim_geometry.py` 已有货架/区域几何常量 | 几何已有，缺 3D 模型 |
| 任务队列 + 优先级 | `app/task_manager.py` 已有 TaskManager（无优先级）；Celery 已在 | 加优先级字段 |
| 多车避障 | 防穿模校验（静态货架）+ `check_path_clear` 已有 | 缺车-车动态避障 |
| Compose 一键启动 | `docker-compose.yml` 已有 | 补前端服务与健康检查 |
| README + 架构图 | README 已较全 | 补架构图与快速上手路径 |
| 录制 Demo | 未开始 | 全新 |
| 接单介绍页 | 未开始 | 全新 |
...
### Three.js 渲染鸟瞰视图（2D 坐标映射 3D）✅

- **现状**：`web/sim.html` 用 Canvas 2D 画平面仓库，坐标映射已有
- **方案**：
...
### SSE 推送小车实时位置 → Three.js 更新 ✅

- **现状**：SSE 机制已有（`main.py` `StreamingResponse(media_type="text/event-stream")`），sim 位置无推送
- **方案**：
...
### 点击地图 → 生成目标坐标 → Agent 执行 ✅

- **现状**：2D canvas 已有 click → 仿真坐标
- **方案**：
...
- **方案**：补前端静态服务（FastAPI 静态挂载 web/）、`SIM_BACKEND` 环境变量、健康检查（API `/health`）；README 增"一键启动"段
...
```
3D 渲染 → SSE 数据流 → 点击交互 → 场景完善
任务队列可与 3D 渲染/SSE 并行
避障依赖点击交互的交互路径，但核心逻辑可先于前端开发
Compose 打包在 3D 功能稳定后
文档/Demo/介绍页最后收尾
```

**风险点**：SSE 与多后端（PyBullet/Gazebo）的采样一致性——`get_all_robots()` 是 `SimBackend` 接口方法，两后端同构，风险低；车-车避障是唯一有算法复杂度的项，先用距离检测 + 中转错峰，不引入全局规划。
