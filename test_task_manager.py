...
def test_upload_with_task():
    ...
    try:
        ...
        with open(TEST_PDF, "rb") as f:
            ...
            resp = requests.post(f"{BASE_URL}/rag/upload", files=files, timeout=30)
...
def test_search_after_upload():
    ...
    for keyword in keywords:
        try:
            resp = requests.get(f"{BASE_URL}/rag/search", params={"q": keyword, "top_k": 3})
            if resp.status_code == 200:
                data = resp.json()
...
def test_invalid_task():
    ...
    try:
        resp = requests.get(f"{BASE_URL}/task/{invalid_id}")
...
def test_clear_knowledge_base():
    ...
    try:
        resp = requests.delete(f"{BASE_URL}/rag/clear")
        if resp.status_code == 200:
