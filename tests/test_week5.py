...
def test_hybrid_search():
    print("\n=== 测试基础混合检索 ===")
    response = requests.get(f"{BASE_URL}/api/rag/v2/search", params={"q": "Docker容器", "top_k": 3})
...
def test_hyde():
    print("\n=== 测试HyDE ===")
    response = requests.get(f"{BASE_URL}/api/rag/v2/ask", 
...
def test_cache():
    ...
    # 第一次查询
    start = time.time()
    response1 = requests.get(f"{BASE_URL}/api/rag/v2/ask", 
                            params={"question": "Docker是什么？"})
    time1 = time.time() - start
    
    # 第二次查询（应该命中缓存）
    start = time.time()
    response2 = requests.get(f"{BASE_URL}/api/rag/v2/ask", 
                            params={"question": "Docker是什么？"})
...
def test_compare():
    print("\n=== 测试方法对比 ===")
    response = requests.get(f"{BASE_URL}/api/rag/compare", 
...
def test_evaluate():
    print("\n=== 测试评估 ===")
    response = requests.get(f"{BASE_URL}/api/rag/evaluate")
    if response.status_code == 200:
        data = response.json()
        print("评估指标:")
